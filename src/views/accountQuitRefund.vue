<template>
  <div class="founderData" style="padding:20px;">
    ...
    <el-table :data="tableData" border height="700" style="width: 100%">
      ...
      <el-table-column inline-template label="可退金额"><span>{{row.availableQuitBalance | rewardMoneys}}</span>
      </el-table-column>
      <el-table-column inline-template label="申请退款金额"><span>{{row.applyQuitBalance | rewardMoneys}}</span>
      </el-table-column>
...
<script>
  ...
  default {
    ...
    methods: {
      ...
      quitRefund(id){
        var self = this;
        $MsgBox.confirm('确定已人工退款?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          this.ajax(this, {
            url: api.AccountQuitRefund,
            type: "post",
            data: {
              id: id
            },
