  ...
  <div class="fundAppList">
    ...
    <el-table :data="tableData" border style="width: 100%">
      ...
      <el-table-column inline-template label="操作" width="400">
        <div>
          ...
          <el-button size="small" v-if="row.status == 'view'" @click="changeStatus('unOnline',row)" :plain="true"
                     type="info">取消预览
          </el-button>
          <el-button size="small" v-if="row.status == 'approved'" @click="changeStatus('online',row)" :plain="true"
                     type="info">在线预览
          </el-button>
          <el-button size="small" @click="changeStatus('publish',row)" :plain="true" type="info"
                     v-if="row.status == 'view'">发布筹款
          </el-button>
          <el-button size="small" @click="changeStatus('forceClose',row)" v-if="row.status != 'finish'" :plain="true"
                     type="info">强制关闭
          </el-button>
    ...
    <el-dialog :title="'强制关闭'+forceClose.name" v-model="forceClose.visible">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="postMyEdit('forceClose','fCloseBack')" :disabled="forceClose.disabled">确定
        </el-button>
      </div>
    </el-dialog>
  </div>
