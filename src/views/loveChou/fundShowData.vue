<template>
  <div class="fundShowData" style="padding:20px">
    ...
    <el-table :data="tableData" border height="700" style="width: 100%">
      <el-table-column label="总数" fixed="left" width="360">
        <el-table-column prop="patientName" label="患者姓名" fixed="" width="120"></el-table-column>
        <el-table-column inline-template prop="status" label="状态" fixed="" width="120">
          <span>{{fundraise[row.status]}}</span></el-table-column>
        <el-table-column prop="fundraiseTargetAmount" label="目标金额" fixed="" width="120"></el-table-column>
      </el-table-column>
      <el-table-column :label="totalNumber.fundraiseAcquiredAmount+''">
        <el-table-column prop="fundraiseAcquiredAmount" label="已筹金额"></el-table-column>
      </el-table-column>
      <el-table-column :label="totalNumber.fundraiseRemitAmount+''">
        <el-table-column prop="fundraiseRemitAmount" label="累计打款金额"></el-table-column>
      </el-table-column>
      <el-table-column :label="totalNumber.fundraiseShareCount+''">
        <el-table-column prop="fundraiseShareCount" label="分享次数" width="120"></el-table-column>
      </el-table-column>
      <el-table-column :label="totalNumber.fundraiseShareOpenCount+''">
        <el-table-column prop="fundraiseShareOpenCount" label="从分享进入次数" width="160"></el-table-column>
      </el-table-column>
      <el-table-column :label="totalNumber.fundraiseDonationClickCount+''">
        <el-table-column prop="fundraiseDonationClickCount" label="点击捐款次数" width="160"></el-table-column>
      </el-table-column>
      <el-table-column :label="totalNumber.fundraiseDonationCount+''">
        <el-table-column prop="fundraiseDonationCount" label="捐款次数" width="120"></el-table-column>
      </el-table-column>
      <el-table-column :label="totalNumber.fundraiseCommentCount+''">
        <el-table-column prop="fundraiseCommentCount" label="评论次数(包括回复)" width="180"></el-table-column>
      </el-table-column>
      <el-table-column :label="totalNumber.openCount+''">
        <el-table-column prop="openCount" label="打开次数" width="120"></el-table-column>
      </el-table-column>
      <el-table-column prop="fundraiseStartTime" label="筹款开始时间" width="180"></el-table-column>
      <el-table-column prop="fundraiseEndTime" label="筹款结束时间" width="180"></el-table-column>
    </el-table>
    <!--分页-->
    <div class="block" style="padding-top:30px;padding-bottom:40px">
      <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
                     :current-page="page.currentPage" :page-sizes="[30, 50, 100, 200]" :page-size="page.currentPageSize"
                     layout="total, sizes, prev, pager, next, jumper" :total="page.totalElement"
                     v-if="page.totalPages>1"></el-pagination>
    </div>
  </div>
</template>
<script>
...
<style>
  ...
  .fundShowData .el-table td.is-hidden > *, .el-table th.is-hidden > * {
    visibility: visible;
  }
</style>
